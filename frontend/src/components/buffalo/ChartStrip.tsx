...
export default function BuffaloChartStrip() {
  const [open, setOpen] = useState(true)
  const { data } = useBuffaloSubcompartments()

  const features = useMemo(
    () => (data?.features ?? []).map((f) => ({ properties: f.properties as unknown as Record<string, unknown> })),
    [data],
  )
  ...
  const year           = useMemo(() => aggregateArea(features, 'year_planted'),    [features])
        ...
        <div className="h-52 flex px-3 py-2">
          <div className="flex flex-col flex-1 min-w-0 px-2">
            ...
            <div className="flex flex-1 gap-3 min-h-0">
              ...
              <MiniChart title="Planting Year"   data={year} />
            </div>
          </div>
        </div>
      )}
